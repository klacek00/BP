...
\vfill\noindent
Havelka, O.: {\it ZLOMKOVÉ DIFERENCIÁLNÍ ROVNICE CAPUTOVA TYPU -- NUMERICKÁ ANALÝZA}, Brno: Vysoké učení technické v Brně, Fakulta strojního inženýrství, 2024. 31 s. Vedoucí bakalářské práce doc. Ing. Petr Tomášek, Ph.D.
...
Problematika neceločíselných derivací je stará téměř jako problematika běžných celočíselných derivací a ukazuje touhu matematiků zobecňovat problémy. Jak se ale v této práci později naznačí, nejedná se o jednoznačné zobecnění, jelikož lze zlomkovou derivaci zadefinovat vícero neekvivalentími způsoby. 
...
Z mnoha vlastností gama funkce budou zmíněny dva vztahy:
...
Faktoriál je tedy pouhým zjednodušením gama funkce a zároveň je možné gama funkci na oboru přirozených čísel uvažovat jako faktoriál. A stejně jako hodnotu faktoriálu lze spočítat pomocí rekurzivní funkce, která využívá pouze zadefinování hodnoty faktoriálu v 0, tak i hodnotu gama funkce s kladným argumentem lze díky větě \eqref{gamma2} rekurzivně spočítat, pokud známe jeho hodnotu na intervalu $\left( 0, 1\right\rangle $.
%\begin{veta}
...
\begin{defin}\label{DefinMLfce} %[Dvouparametrická Mittag-Lefflerova funkce]
...
%\begin{defin} [Tříparametrická Mittag-Lefflerova funkce]
...
\medskip
\noindent Přirozenou otázku u sumární definice Mittag-Lefflerovy funkce vyvolává její definiční obor, respektive obor konvergence mocninné řady. 
Pro stanovení oboru konvergence lze použít známé limitní podílové kritérium. Parametr $\rho$ lze vypočítat jako podíl po sobě jdoucích členů $a_k$ nekonečné řady v~limitě, kdy $k \rightarrow \infty $.
...
Pro poloměr konvergence $R$ platí, že se jedná o inverzní hodnotu k parametru $\rho$. V případě hodnoty parametru $\rho = 0$ se pokládá $R = \infty$.
...
\medskip
\noindent V této části bude blíže rozebrána možnost výpočtu hodnot Mittag-Lefflerovy funkce. Jelikož je samotná funkce zadefinovaná jako mocninná řada \eqref{JednoporametrickaMLfce}, nabízí se tuto řadu sečíst.
...
Označme Err chybu  $n$-tého částečného součtu Mittag-Lefflerovy funkce, tj.
\begin{equation} \label{ChybaSouctuRady}
	Err_{n+1} \left(E_{\alpha, \beta} \left( z\right)\right) = \lim_{L\to \infty} \sum_{k=n+1}^{L} \frac{z^{k}}{\Gamma\left( \alpha k +\beta \right)} \space \space \left( \alpha > 0, \beta > 0 \right).
\end{equation}

Mocninnou řadu \eqref{ChybaSouctuRady} se tedy pokusíme shora omezit pomocí podobné geometrické řady, na jejíž součet postačí středoškolský vzoreček:

\begin{equation}\label{SoucetGeometrickeRady}
	S = \sum_{k=0}^{\infty} a_{0}\cdot q^{k} = \frac{a_{0}}{1-q},
\end{equation} 
kde značíme součet geometrické řady $S$, řadu $A$ $\left[a\right]_{0}^{\infty}$ s~kvocientem $q$ a prvním čleemn řady $a_0$. Přičemž $ |q| < 1$.
...
Výpočet pomocí for-cyklu a upravením pořadí násobení a dělení
 ...
 Je zřejmé, že při tomto výpočtu se dvě metody řešení neshodují. Absolutní hodnota rozdílu těchto hodnot je $3.509485382540145 \cdot 10^{-3} $, a výsledky obou metod se tedy neshodují. % a relativní rozdíl těchto vypočtených hodnot je $\approx 5.865102640704210\cdot 10^{-3}$. 
...
\end{equation}
Vztahy \eqref{Laplace}, \eqref{inverseLaplace} definují integrální problém, který je nutné následně řešit pokročilými numerickými metodami.
...
Tato práce je téměř výhradně omezena na levou Caputovu derivaci definovanou na intervalu $\left(a, t\right)$. Ve všech případech, kdy bude dále zmíněna Caputova derivace, bude tím myšlena první rovnice z~derivací, pokud nebude výslovně řečeno jinak.
...
\end{equation}
kde $_{a}^{RL}D^{\alpha}_{t} y\left(t\right)$ značí Riemann-Liouvillovu derivaci řádu $\alpha$ a $_{a}^{C}D^{\alpha}_{t} y\left(t\right)$ značí Caputovu derivaci řádu $\alpha$. Dále platí, že $m \in \N: m-1<\alpha<m$ a $y\in C^{m-1}\left<a, t\right>$ a funkce $y^{\left(m\right)}$ je integrovatelná na $\left< a, t \right>$.
...
\end{equation} 
	kde $\Phi \left(t\right) = \sum_{k=0}^{m-1} \frac{f^{\left(k\right)}\left(a\right)}{\Gamma \left(k+1\right)} \left(t-a\right)^{k}$.
...
Ze vztahů \eqref{NecelociselnaxCelociselna} vyplývá, že Riemann-Liouvillova derivace se v limitním případě rovná celočíselné derivaci. Toto však v obecnosti neplatí pro Caputovu derivaci.
...
\begin{veta}[Existence řešení počáteční úlohy zlomkové diferenciální rovnice]\label{ExistenceFDE}
	...
	Mějme okolí $\mathcal{D}$ takové, že $\mathcal{D} := \left\langle0,\mathcal{X} ^{*}\right\rangle \times \left\langle u_{0}^{0} - \delta , u_{0}^{0} + \delta \right\rangle $, přičemž $\mathcal{X}^{*}>0$ a $ \delta  > 0$. A nechť je funkce $f: \mathcal{D} \to \R$ je spojitá.
	
	Pak existuje funkce $u:\left\langle 0, \mathcal{X}\right\rangle \to \R$, která je řešením počáteční úlohy \eqref{PocUloha}.
\end{veta} 
...
Jak je zmíněno v  \cite{NumMetody}, spojitá funkce  $y\left(t,x\right)$ na okolí $\mathcal{D}$ bodu $\left[a, \eta\right]$ splňuje Lipschitzovu podmínku vzhledem k~proměnné $x$, jestliže % $\Leftrightarrow$ 
$$ |y\left(t, b\right)- y\left( t, a\right)| \leq L \cdot |b-a| \qquad  \forall \left[t,b\right], \left[t,a\right]  \in \mathcal{D}.$$

\begin{veta} [Jednoznačnost řešení počáteční úlohy zlomkové diferenciální rovnice]
	Mějme okolí $\mathcal{D}$ takové, že $\mathcal{D} := \left\langle 0,\mathcal{X} ^{*}\right\rangle \times \left\langle y_{0}^{0} - \delta , y_{0}^{0} + \delta \right\rangle $ přičemž $\mathcal{X}^{*} > 0$ a $\delta > 0$. A nechť je funkce $f: \mathcal{D} \to \R$ ohraničená na $\mathcal{D}$ a splňuje výše zmíněnou Lipschitzovu podmínku vzhledem k druhé proměnné. A~pokud předpoklad věty \ref{ExistenceFDE} platí, pak existuje právě jedna funkce $y:\left\langle0, \mathcal{X} \to \right\rangle \R$, která je řešením počátečního problému \eqref{PocUloha}.
\end{veta}
...
\end{equation}
kde $\omega = \sqrt[]{\frac{k}{m}}$ je úhlová frekvence kmitání a $\varphi = \arctan{\frac{c_{0}}{c_{1}}}$. Viz \cite{HRW} \cite{Fyzika1Kulhanek}.

V praxi se však střetáváme s vnitřním a vnějším třením, mezi které lze zařadit například odpor vzduchu. Vlivem těchto se pohyb harmonického oscilátoru utlumuje. Zlomkové diferenciální rovnice umožňují tyto tlumené kmity dobře modelovat modifikací druhé derivace v diferenciální rovnici:
...
\end{equation}
Pro rovnici \eqref{FractionalOscilator} máme $\lceil \alpha \rceil = 2$, tedy \eqref{LaplaceFracDerr} je tvaru: 
\begin{equation}
...
\end{equation*}
kde $m$ je hmotnost tělesa, $m\cdot {\dx ^{2} y\left(t\right)}/{\dx t^{2}}$ je setrvačná síla tělesa, $c\cdot \space _{0}D^{\alpha}_{t} y\left(t\right)$ je tlumící síla představující ztráty třením, $k$ je tuhost systému, $ky\left(t\right)$ je pružná síla a $f\left(t\right)$ je funkce vnější síly působící na těleso a systém, viz \cite{ChenInMechanics}.
Numerické řešení takovéto rovnice je však nad rámec této práce a je nutné pro řešení využít pokročilejších numerických metod.
...
%V této kapitole budou nejprve uvedeny základní pojmy pro numerická řešení zlomkových diferenciálních rovnic a poté budou  představeny L1 a L2 metody výpočtu Caputovy derivace a~Eulerovy metody pro řešení počátečních úloh zlomkových diferenciálních rovnic. %opakuje se
\subsection{Základní pojmy numerického počtu} \label{zakladni pojmy}
Nejprve připomeneme formulaci počáteční úlohy \eqref{PocUloha}, kterou budeme numericky řešit:
\begin{equation*}
	\begin{aligned}
		^{C}_{0}D^{\alpha}_{t} y \left(t\right) &= f\left(t,y\left(t\right)\right), m-1 < \alpha < m, m \in \N, \\
		y^{\left(j\right)} \left(0\right) &= y_{0}^{j}, j=0,1,..., m-1.
...
Aproximaci řešení zlomkové diferenciální rovnice v bodě $t_n$ označíme $y_n$, zatímco přesné řešení značíme $y\left(t_n\right)$. %Pokud bychom chtěli najít řešení mimo naši síť $t_n$, lze buď zhustit síť $t_n$, aby pokrývala více bodů, nebo je nutné použít vhodnou interpolaci pomocí okolních bodů.

Řešení počátečního problému pro diferenciální rovnice se v numerickém počtu zpravidla převádí na problém diferenčních rovnic. Tato přibližná náhrada je ovšem zatížena chybou. Chyb numerických metod existuje celá řada, zde budou uvedeny některé z nich.% V každém kroku $n$ o velikosti $\tau$ tohoto zjednodušení dochází k chybě. V numerické matematice zavádíme několik druhů chyb.

\begin{defin}\label{GlobalniDiskretizacniChyba}
	\textit{Globální diskretizační chybu} v kroku $n$ (zn. $\varepsilon_{n}$) je dána vztahem:
...
%\begin{defin}
	%\emph{Lokální diskretizační chybu} v kroku $n$ (zn. $lte_{n}$, anglicky "local truncation error") se vypočítá jako:
...
% lte chybu získáme za tzv lokalizačního předpokladu, tedy, že hodnoty řešení ve všech předchozích uzlech jsou přesné, je rozdíl levé a pravé strany explicitní numerické formule

\begin{defin}
	\emph{Lokální chyba} v kroku $n$ (zn. $le_{n}$, anglicky "local error") je dána vztahem:
	$$le_{n} = y\left(t_{n+1}\right) - y_{n+1}.$$
	%$y_{n+1}$ je numerické řešení v bodě $t_{n+1}$ a $y_{n}\left(t\right)$ je analytické řešení počátečního problému \eqref{PocUloha}
	%$$y'_{n}\left(t\right) = f\left(t,y_{n}\left(t\right)\right), \qquad y_{n}\left(t_{n}\right) = y_{n}.$$
\end{defin}

Existuje také lokální diskretizační chyba, která značí chybu v daném kroku výpočtu, za předpokladu, že všechny předchozí kroky jsou přesné. Tuto chybu budeme definovat samostatně u~některých metod.
...
\begin{defin} [\emph{EOC}] \label{EOC}
	Výrazem \emph{EOC} ("Estimated Order of Convergence") označíme tzv. {\emph{odhad řádu numerické metody}}, který je dán vztahem:
...
\end{equation}
kde $b_k$ je definované vztahem% \eqref{b_k}
...
\end{equation}
	...
	\end{equation}
kde člen $W_{k}$ je určen pro dané $k$ vztahem% \eqref{W_k}
...
Pro níže uvedené Eulerovy metody využíváme dvouparametrický člen:
...
Uvažujme počáteční úlohu zlomkových diferenciálních rovnic \eqref{PocUloha}. Pak
\begin{enumerate}[label=(\textbf{\arabic*})]
	\item \textbf{Explicitní Eulerova metoda:}
	...
	\item \textbf{Implicitní Eulerova metoda:}
	...
	\item \textbf{Theta metoda:} 
			$\left[_{0}D_{t}^{-\alpha} f\left(t,y\left(t\right)\right)\right]_{t=t_{n+1}}$ je aproximována váženou zlomkovou obdélníkovou formulí:
...
%Nyní budou uvedeny některé výsledky ohledně řádu chyby výše zmíněných numerických metod.%V další části bude pojednáno o řádu chyby výše zmíněných numerických metod.

\begin{lem} \label{LemmaOGlobChybe}
	Uvažujme počáteční úlohu \eqref{PocUloha} a numerické metody \eqref{C-PrimyEuler} \eqref{C-PrimyEuler} a \eqref{C-VazenaMetoda}.	Nechť $\alpha$~je řád derivace, $\tau$ je  krok na ekvidistantní síti,$k \in N: k\tau\leq b$, viz \ref{zakladni pojmy}, $M> 0$ je konstanta a $b$ je koncový bod intervalu pro hledání řešení.
	\begin{equation*}
	...
	Pokud pro chybový členy $\varepsilon_n$ %\ref{}
	\begin{equation*}
...
%$f \left(t,y \left(t\right)\right)$, kde
\begin{veta} \label{RadEulerovyMetody}
	Nechť funkce $y\left(t\right)$, která je řešením počátečního problému \eqref{PocUloha}, je lipschitzovská na celém intervalu $I$ a také  $y\left(t\right) \in C^{1}\left\langle 0, T\right\rangle$,  tedy, má spojité derivace. A
	$y_{n}, \left(1 \leq n \leq N\right)$ jsou numerická řešení theta metody pro diferenciální rovnice \eqref{C-VazenaMetoda}. Pak platí:
	
	\begin{equation*}
		|y\left(t_{n+1}\right) - y_{n+1}| \leq C\cdot \tau, \qquad n = 0,1, ..., N-1,
	\end{equation*} 
	kde $C>0$ je nezávislá na velikosti kroku $\tau$  a $k$. Pak řekneme, že theta metoda \eqref{C-VazenaMetoda} je tedy metoda prvního řádu $O\left(\tau\right)$. Viz \cite{Cangpin}.
\end{veta}
%Provedeme důkaz věty \ref{RadEulerovyMetody}. Pro dostatečně malý krok $\tau$:
\begin{proof}%né metody z \ref{RadEulerovyMetody}]
	Nechť velikost globální chyby $|\varepsilon_{n}| = |y\left(t_{n}\right) - y_{n}|$.
	...
	\end{equation*}
	(To lze zvolit protože $L_{2}$ je konstanta nezávislá na kroku $\tau$, $\alpha$ závisí pouze na počáteční úloze a $\theta$  je volený parametr Theta metody.)
	Lze zavést
	...
	\end{equation*}
	Aplikací Lemma \ref{LemmaOGlobChybe} je pak tvrzení dokázáno. 
\end{proof}

Jelikož parametr $\theta$ v theta metodě \eqref{C-VazenaMetoda} zobecňuje explicitní Eulerovu metodu \eqref{C-PrimyEuler} i implicitní eulerovu metodu \eqref{C-NeprimyEuler} díky volbě $\theta = 1$ a $\theta = 0$. Pak také platí, že explicitní Eulerova metoda \eqref{C-PrimyEuler} a implicitní Eulerova metoda \eqref{C-NeprimyEuler} jsou také metody prvního řádu $O\left(\tau\right)$.
\\

Důležité téma numerického počtu je stabilita metody. Numerickou metodu nazveme stabilní, jestliže malé změny v počátečních podmínkách nevyvolají velké změny v řešení rovnic. Pro tuto problematiku zmíníme jednu z vět uvedenou v \cite{Cangpin}, kde se laskavý čtenář může o stabilitě numerických metod pro zlomkové diferenciální rovnice dozvědět více. 
...
Předpokládejme splnění předpokladů věty \ref{RadEulerovyMetody} pro parametr $\theta = 0$ nebo $\theta = 1$. Pak řešení počátečního problému \eqref{PocUloha} implicitní nebo explicitní Eulerovou metodou je stabilní.
...
Výše uvedené testovací úlohy budeme aplikovat na několik počátečních úloh.Ověříme , zda výše zmíněné metody skutečně aproximují řešení zlomkových diferenciálních rovnic, jestli výpočet na jemnější síti má pozitivní vliv na velikost chyby, jak se chová výpočet theta metody pro různé velikosti $\theta$, a v závěru i ověřit pomocí EOC řád konvergence numerické metody.
Analytické řešení testovacích úloh bylo ověřeno pomocí softwaru Maple. 
...
\begin{prikl}\label{TestovaciUloha1}
	...
	Analytické řešení této diferenciální rovnice je%  uvedeno v \cite{LiYe}
	\begin{equation*}
		y \left(t\right) = t^{2}.
	\end{equation*}
	Viz \cite{LiYe}.
...
\begin{prikl}\label{TestUlohaOsciator}
	...
	Jak bylo ukázáno výše, rovnice má analytické řešení \eqref{LaplaceMittagLeffler}, konkrétně pro tuto úlohu:

	\begin{center}
		$	y\left(t\right) = E_{\alpha, 1}\left(-t^{\alpha}\right).$
	\end{center}%\begin{flushright}\eqref{LaplaceMittag-Leffler} \end{flushright}	

%	Kde  $E_{\alpha, \beta}\left(z\right)$ je Mittag-Lefflerova funkce.
	Konstanty $c_{0}, c_{1}$ jsou dány počátečními  podmínkami $c_{0} = y\left(0\right) = 1, c_{1} = y'\left(0\right) = 0$.% a parametry $m, k$ definovanými výše. 
	...
	Výsledky jsou uvedeny v tabulce \ref{tab:tabulkaTestovacíÚloha(2),alpha=1.1} pro řád derivace $\alpha = 1.1$, v tabulce \ref{tab:tabulkaTestovacíÚloha(2),alpha=1.5} pro řád derivace $\alpha = 1.5$ a v tabulce \ref{tab:tabulkaTestovacíÚloha(2),alpha=1.9} pro řád derivace $\alpha = 1.9$. 
	
	V tabulkách \ref{tab:tabulkaTestovacíÚloha(2),alpha=1.5} a \ref{tab:tabulkaTestovacíÚloha(2),alpha=1.9} pro příklad \ref{TestUlohaOsciator} pro řád po řadě $\alpha = 1.5$ a~$\alpha = 1.9$  je vidět přesný odhad pro řád numerických metod (připomeňme, že theta metoda pro $\theta = 0$ je shodná s implicitní metodou a  $\theta = 1$ je shodná s explicitní Eulerovou metodou). Pro výpočet s derivací $\alpha =1.1$ a parametrem $\theta = 0.7$ a $\theta = 1$ vychází vyšší odhad řádu konvergence EOC, což může být způsobeno specifikem příkladu a zvolených parametrů, nedostatečnou přesností výpočtu přesného řešení nebo pro vyšší počet kroků $N$ i zaokrouhlovacími chybami.
	...
	\begin{tabular}{|c||cc|cc|cc|cc|}
		...
		\hline
		200   & $5.24 \cdot 10^{-5}$ & $1.0071$ & $1.47 \cdot 10^{-5}$ &$0.8005$& $1.31 \cdot 10^{-5}$ &$1.3024$ &$4.07 \cdot 10^{-5}$&$1.1303$\\
		\hline
...
\end{equation}
Úlohou je určit Caputovu derivaci funkce \eqref{TestUlohaCaputovaDerivace} řádu $0.3, 0.5$ a $0.8$ v bodě $t = 2$.

K nalezení analytického řešení pro tuto úlohu byl použit software Maple. Hodnota funkce v~bodě $t = 2$ byla vyčíslena na 20 platných dekadických cifer.% Hledanou funkci nazveme funkcí $Y$, přičemž spodní index značí stupeň derivace.
Označme $Y_{0.3}\left(t\right)$, $Y_{0.5}\left(t\right)$, $Y_{0.8}\left(t\right)$ po řadě derivaci řádu $0.3$, $0.5$ a $0.8$.
\begin{equation*}
	\begin{aligned}
...
Numerické řešení příkladu \ref{TestUlohaL1Metoda} je uvedeno v tabulce \ref{tab:tabulkaTestovacíÚloha(3),alpha=0.3-0.8}. Lze pozorovat poměrně dobré aproximace řešení i pro nízký počet kroků. Pro všechny tři hodnoty $\alpha$ a pro počet kroků $N = 1600$ je globální chyba řešení menší než $\varepsilon < 10^{-3}$. Pro všechny hodnoty parametrů je vidět vcelku přesný odhad řádu konvergence, jelikož L1 metoda je řádu $2 - \alpha$.
...
\newpage
\section{Závěr}
Cílem této práce bylo nastudovat problematiku zlomkových diferenciálních rovnic, realizovat numerické metody a diskutovat vlastnosti daných numerických metod.

V první části byla představena teorie potřebná k zavedení zlomkových diferenciálních rovnic. Tato část vrcholí ukázkovým výpočtem úlohy mechanického oscilátoru. Zároveň byl diskutován algoritmus pro výpočet funkčních hodnot Mittag-Lefflerovy funkce a pro následnou práci byl vybrán stabilnější postup řešení. 

Hlavní část bakalářské práce byla zaměřena na numerické řešení zlomkových diferenciálních rovnic, kdy všechny zmíněné metody byly realizovány v prostředí Matlab, zejména se jedná o~Eulerovy implicitní a explicitní metody a theta metodu. Výčet přiložených programů je uveden v~seznamu příloh, včetně skriptů obsahujících testovací úlohy s analýzou řádu konvergence metod. Některé z daných úloh byly také rozebrány přímo v textu této práce.

Oblast zlomkových diferenciálních rovnic je obsáhlým a stále se rozvíjejícím odvětvím. Nebylo tedy zdaleka možné popsat vše a zůstalo mnoho možností práci rozvinout. Zejména by se mohlo přikročit k upravení kódu v softwaru Matlab pro řešení soustav zlomkových diferenciálních rovnic. Přirozená výpočetní náročnost algoritmů v sobě také obsahuje velký prostor pro větší optimalizaci, paralelizaci výpočtů nebo potlačování zaokrouhlovacích chyb. Dále by bylo možné rozšířit soubor metod o řešení vícekrokovými metodami nebo o metody vyššího řádu konvergence. 
...
\bibitem{NumMetody} ČERMÁK, Libor. Numerické metody [online]. Brno: Vysoké učení technické v Brně, 2020 [cit. 2023-05-19]. Dostupné z: https://mathonline.fme.vutbr.cz/Numericke-metody-II/sc-1246-sr-1-a-263/default.aspx	
...
\bibitem{HRW} HALLIDAY, David, Robert RESNICK, Jearl WALKER, et al. Brno: Vysoké učení technické v Brně - nakladatelství VUTIUM, 2021. Překlady vysokoškolských učebnic. ISBN 978-80-214-4123-1.
...
\bibitem{LeibnitzFail} WEISS, George H. R. Klages, G. Radons, I.M. Sokolov (eds.): Anomalous Transport. Online. Journal of statistical physics. 2009, roč. 135, č. 2, s. 389-391. ISSN 0022-4715. Dostupné z: https://doi.org/10.1007/s10955-009-9713-5. [cit. 2024-04-09].
...
\begin{enumerate}
	\item \textbf{L1Caputo.m}
	\begin{itemize}
		\item MATLAB funkce pro výpočet Caputovy derivace pomocí L1 metody.
	\end{itemize}
	
	\item \textbf{L2Caputo.m}
	\begin{itemize}
		\item MATLAB funkce pro výpočet Caputovy derivace pomocí L2 metody.
	\end{itemize}
	
	\item \textbf{FractionalBackwardEuler.m}
	\begin{itemize}
		\item MATLAB funkce pro řešení zlomkové diferenciální rovnice pomocí Implicitní Eulerovy metody.
	\end{itemize}
	
	\item \textbf{FractionalForwardEulerOptimalized.m}
	\begin{itemize}
		\item MATLAB funkce pro řešení zlomkové diferenciální rovnice Explicitní Eulerovou metodou.
	\end{itemize}

	\item \textbf{FractionalWeightedEuler.m}
	\begin{itemize}
		\item MATLAB funkce pro řešení zlomkové diferenciální rovnice theta metodou.
	\end{itemize}
	
	\item \textbf{FractionalOscillator.m}
	\begin{itemize}
		\item MATLAB skript pro simulaci mechanického oscilátoru.
	\end{itemize}

	\item \textbf{L1MetodTester.m}
	\begin{itemize}
		\item MATLAB skript pro testování L1 metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		\item $y = 2t - 3t^{2} + t^{3}$;
	\end{itemize}

	\item \textbf{L2MetodTester.m}
	\begin{itemize}
		\item MATLAB skript pro testování L2 metody pro řád derivace $\alpha \in \left(1,2\right)$. 
		\item $y = 2t - 3t^{2} + t^{3}$;
	\end{itemize}
	
	\item \textbf{TestUlohyEuler.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = \frac{2}{\Gamma(2.5)} t^{1.5} - y + t^2$, $y_{0} = 0$
		
	\end{itemize}
	
	\item \textbf{TestUlohyEuler2.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = -y$, $y_{0} = 0$
	\end{itemize}
	
	\item \textbf{TestUlohyEuler3.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		    \item$^{C}_{0}D^{\alpha}_{t} y\left(t\right) = \frac{2}{\Gamma(3 - \alpha)} t^{2 - \alpha} - \frac{1}{\Gamma(2 - \alpha)} t^{1 - \alpha} - y + t^2 - t$, $y_{0} = 0$
		
	\end{itemize}
	
	%\item \textbf{TestUlohyEuler4.m}f
	%\begin{itemize}
	%	\item MATLAB skript pro testování úlohy Eulerovy metody 4.
	%	        \item $y^{\left(\alpha\right)} = -y^2 - \frac{2}{(t + 1)^2}$, $y_{0} = 0$		
	%\end{itemize}
	
	\item \textbf{TestUlohyEuler5.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = t^2$, $y_{0} = 0$
		    
		
	\end{itemize}
	
	\item \textbf{TestUlohyEuler6.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(0,1\right)$. 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = y^{0.2}$, $y_{0} = 0$
		
	\end{itemize}
	
	\item \textbf{TestUlohyEuler7.m}
	\begin{itemize}
		\item MATLAB skript pro testování úlohy Eulerovy metody pro řád derivace $\alpha \in \left(1,2\right)$. 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = -y^2 + \frac{\Gamma(6)}{\Gamma(6 - \alpha)} t^{5 - \alpha} - \frac{3 \Gamma(5)}{\Gamma(5 - \alpha)} t^{4 - \alpha} + \frac{\Gamma(5)}{\Gamma(4 - \alpha)} t^{3 - \alpha} + \left(t^5 - 3 t^4 + 2 t^3\right)^2$,\\ $y_{0} = 0$, $y'_{0} = 0$
		
	\end{itemize}

	\item \textbf{TestUlohyGarapa.m}
	\begin{itemize}
		\item MATLAB skript pro testovací úlohu z práce R. Garrapy 
		\item $^{C}_{0}D^{\alpha}_{t} y\left(t\right) = \frac{40320}{\Gamma(9 - \alpha)} t^{8 - \alpha} - \frac{3 \Gamma\left(5 + \frac{\alpha}{2}\right)}{\Gamma\left(5 - \frac{\alpha}{2}\right)} t^{4 - \frac{\alpha}{2}} + \frac{9}{4} \Gamma(\alpha + 1) + \left(\frac{3}{2} t^{\frac{\alpha}{2}} - t^4\right)^3 - y^{\frac{3}{2}}$, $y_{0} = 0$	
	\end{itemize}
