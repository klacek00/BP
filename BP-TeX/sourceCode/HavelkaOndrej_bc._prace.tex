...
Nejprve připomeneme počáteční úlohu \ref{PocUloha}, která byla již dříve důkladně rozebrána.
\begin{equation*}
	\begin{aligned}
		^{C}_{0}D^{\alpha}_{t} y \left(t\right) &= f\left(t,y\left(t\right)\right), m-1 < \alpha < m, m \in \N, \\
		y^{\left(j\right)} \left(0\right) &= y_{0}^{j}, j=0,1,..., m-1.
	\end{aligned}	
\end{equation*}
Zatímco $y\left(t\right)$, kde $t \in I = \left\langle0, b\right\rangle$ označuje přesné řešení počáteční úlohy. Úlohou numerického počtu v počáteční úloze zlomkových diferenciálních rovnic je najít co nejlepší řešení v konečně mnoha bodech intervalu $\left(0,B\right\rangle$. 
...
Řešení počátečního problému pro diferenciální rovnice se v numerickém počtu zjednodušuje na na problém diferenčních rovnic. V každém kroku $n$ o velikosti $\tau$ tohoto zjednodušení dochází k chybě. V numerické matematice zavádíme několik druhů chyb. %TODO zavést síť tau
...
Řešení příkladu \ref{TestUlohaL1Metoda} je zapsáno v tabulce \ref{tab:tabulkaTestovacíÚloha(3),alpha=0.3-0.8}. Z vypočtených hodnot řešení je vidět velmi přesné řešení problému i pro nízký počet kroků. Pro všechny tři hodnoty $\alpha$ a pro počet kroků $N = 1600$ je globální chyba řešení menší než $\varepsilon < 10^{-3}$. Pro všechny úlohy je vidět vcelku přesný odhad řádu konvergence, jelikož L1 metoda je řádu $2 - \alpha$. 
