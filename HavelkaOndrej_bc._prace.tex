...
% TODO úvod košatěji
%TODO jména matematiků v plné délce
V následujících staletích se svými definicemi přicházeli postupně Euler (1730), Fourier (1820-1822), Abel (1823-1826), Liouville (1832-1855), Riemann  (1847-1876) Caputo (1967)  a další. 
V~dnešní době se pak nejčastěji využívají Riemann-Liouvillova derivace a Caputova derivace, na kterou bude práce zejména zaměřena. 
...
Zlomkové diferenciální úlohy hrajou roli ve velkém množství lidské činnosti. Jako jedna z historicky prvních úloh spojená se zlomkovou derivací se dá považovat úloha tautochrony, tedy křivky po které je doba "klouzání" hmotného bodu $ T$ nezávislá na počáteční poloze. Touto úlohou se zabýval Abel, který popsal tento problém integrální rovnice:
%TODO  iniciály Abela
...
\subsection{Gama funkce}
Nejprve je vhodné zmínit gama funkci. Gama funkce je funkcí zobecňující faktoriál, avšak stejně jako se neceločíselná derivace zbavuje pout danými reálnými čísly, i gama funkce se dostává mezi čísla reálná. V případě gama funkce je sice definovaná i na oboru komplexních čísel, avšak pro účely této práce je zobecnění na reálná čísla více než dostatečné.
 
\begin{defin}{\emph{Gama funkce }} \label{GammaFunkce}
	Nechť $z \in \C$, kde $\space Re\left(z\right) > 0$, pak  hodnota \emph{gama funkce} v bodě z je dána vztahem
	$$
	\Gamma \left( z\right)  = \int_{0}^{ \infty } t^{z-1} \cdot e^{-t} \dx{t}, \qquad  Re\left( z\right)  > 0
	$$ \cite{Bashour}
\end{defin}
Jedná se pouze o jednu z nejčastěji uváděných definic, lze zavést i definici obecnější, která by pokrývala i obor záporných čísel, pro účely této práce však bude tato definice dostatečná. %přesunout výše a spojit
...
\subsection{Mittag-Lefflerova funkce}
...
\subsection{Definiční obor Mittag-Leflerovy funkce}
...
\subsection{Výpočet funkčních hodnot Mittag-Lefflerovy funkce}\label{DefOMLfce}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Zlomková derivace a integrál}
...
\subsection{Definice zlomkové derivace a integrálu}
...
\subsection{Vztahy mezi neceločíselnou a celočíselnou derivací}
...
\subsection{Další vlastnosti zlomkové derivace}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Zlomkové diferenciální rovnice}
...
\section{Využití zlomkových diferenciálních rovnic}
...
\subsection{Tlumený harmonický oscilátor} \label{TlumenyHarmonickyOscilator}
...
\begin{equation}
	\begin{aligned}
		Y_{0.3}\left(t\right) = ^{C}_{0}D^{0.3}_{t} y\left(t\right) &\approx  1.438624059 x^{\frac{27}{10}}- 3.884284960 x^{\frac{17}{10}}+ 2.201094810 x^{\frac{7}{10}} \\
		Y_{0.3}\left(2\right)	&\overset{.}{=}  0.30381671552548292912	
	\end{aligned}
\end{equation}
\begin{equation}
	\begin{aligned}
		Y_{0.5}\left(t\right) = ^{C}_{0}D^{0.5}_{t} y\left(t\right) &\approx   1.805406667 x^{\frac{5}{2}}- 4.513516668 x^{\frac{3}{2}}+ 2.256758334 \mathrm{\sqrt{x}}, \\
		Y_{0.5}\left(2\right)	&\overset{.}{=}   0.63830764745689136385,
	\end{aligned}
\end{equation}
\begin{equation}
	\begin{aligned}
		Y_{0.8}\left(t\right) = ^{C}_{0}D^{0.8}_{t} y\left(t\right) &\approx   \frac{\Gamma \! \left(\frac{4}{5}\right) \left(\frac{125 x^{\frac{11}{5}}}{11}-25 x^{\frac{6}{5}}+10 x^{\frac{1}{5}}\right)}{\pi  \csc\! \left(\frac{\pi}{5}\right)} \\
		Y_{0.8}\left(2\right) =\frac{60 \Gamma \! \left(\frac{4}{5}\right) 2^{\frac{1}{5}}}{11 \pi  \csc \! \left(\frac{\pi}{5}\right)}	&\overset{.}{=}  1.3648095609347007592
	\end{aligned}
\end{equation}
...
\end{prikl}
	...
	\begin{tabular}{|c||cc|cc|cc|}
		...
		\hline
		800   	& $3.28 \cdot 10^{-5}$ & $1.6623$	& $1.72 \cdot 10^{-4}$ & $1.4865$	& $1.72 \cdot 10^{-3}$ & $1.1972$\\
		\hline
		1600  	& $1.03 \cdot 10^{-5}$ & $1.6771$	& $6.13 \cdot 10^{-5}$ & $1.4806$	& $7.49 \cdot 10^{-4}$ & $1.1985$\\
		\hline
		3200  	& $3.22 \cdot 10^{-6}$ & $1.6838$	& $2.18 \cdot 10^{-5}$ & $1.4935$	& $3.26 \cdot 10^{-4}$ & $1.1991$\\
		\hline
		6400  	& $1.00 \cdot 10^{-6}$ & $1.6938$	& $7.72 \cdot 10^{-6}$ & $1.4956$	& $1.42 \cdot 10^{-4}$ & $1.1995$\\
		\hline
		12800 	& $3.10 \cdot 10^{-7}$ & $1.7172$	& $2.73 \cdot 10^{-6}$ & $1.4972$	& $6.19 \cdot 10^{-5}$ & $1.1998$\\
		\hline
		25600 	& $9.42 \cdot 10^{-8}$ & $1.7896$	& $9.67 \cdot 10^{-7}$ & $1.4989$	& $2.69 \cdot 10^{-5}$ & $1.1999$\\
		\hline
		51200   & $2.73 \cdot 10^{-8}$ & $2.0707$	& $3.42 \cdot 10^{-7}$ & $1.5016$ 	& $1.17 \cdot 10^{-5}$ & $1.2001$\\
		\hline
		102400   & $6.49 \cdot 10^{-9}$ & $6.8216$	& $1.20 \cdot 10^{-7}$ & $1.5080$ 	& $5.10 \cdot 10^{-6}$ & $1.2003$\\
		\hline
		204800   & $5.74 \cdot 10^{-11}$ & $-5.0735$& $4.18 \cdot 10^{-8}$ & $1.5255$ 	& $2.22 \cdot 10^{-6}$ & $1.2008$\\
		\hline
		409600   & $1.93 \cdot 10^{-9}$ & $0.6951$	& $1.40 \cdot 10^{-8}$ & $1.5764$ 	& $9.64 \cdot 10^{-7}$ & $1.2020$\\
		\hline
